...
class MoviesInteractorImpl(private val repository: MoviesRepository) : MoviesInteractor {
    ...
    override fun getMovieDetail(id: String, consumer: MoviesInteractor.MoviesDetailConsumer) {
        executor.execute {
            repository.getDetails(id).handle(object : HandleResult<MovieDetail> {
                override fun handleSuccess(data: MovieDetail?) {
                    consumer.consume(data, null)
                }

                override fun handleError(message: String?, data: MovieDetail?) {
                    consumer.consume(null, message)
                }
            })
        }
